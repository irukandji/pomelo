...
Application.init = function(opts) {
  ...
  var cfgBase = opts.cfgBase || base;
  this.set(Constants.RESERVED.BASE, base, true);
  this.set(Constants.RESERVED.CFG_BASE, cfgBase, true);
...
Application.getConfigBase = function() {
  return this.get(Constants.RESERVED.CFG_BASE);
};
...
Application.loadConfigBaseApp = function (key, val, reload) {
  var self = this;
  var env = this.get(Constants.RESERVED.ENV);
  var originPath = path.join(Application.getConfigBase(), val);
  var presentPath = path.join(Application.getConfigBase(), Constants.FILEPATH.CONFIG_DIR, env, path.basename(val));
  var realPath;
  if(fs.existsSync(originPath)) {
     realPath = originPath;

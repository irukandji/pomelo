...
pro.send = function(reqId, route, msg, recvs, opts, cb) {
  if (this.app.get('env') === 'development') {
      logger.debug('send message reqId: %s, route: %s, msg: %j, receivers: %j, opts: %j', reqId, route, msg, recvs, opts);
  }
...
var bindEvents = function(self, socket) {
  ...
  var closed = false;
  self.app.event.emit(events.CREATE_SESSION, session);
...
var getSession = function(self, socket) {
  ...
  logger.debug('session is created with session id: %s', sid);
  ...
  session.on('bind', function(uid) {
    logger.debug('session bind with uid: %s', uid);
...
var handleMessage = function(self, session, msg) {
    if (self.app.get('env') === 'development') {
        logger.debug('handleMessage session id: %s, msg: %j', session.id, msg);
    }
...
var verifyMessage = function(self, session, msg) {
  ...
  if (!sig) {
    logger.error('receive data from client has no signature');
    return false;
  }
  ...
  if (!pubKey.n || !pubKey.e) {
    logger.error('could not verify message without public key');
    return false;
  }

...
var Socket = function(id, socket) {
    ...
    this.appMonitor = app.components.__app_monitor__;
...
Socket.prototype.sendRaw = function(msg) {
    ...
    this.socket.send(msg, {binary: true}, function(err) {
        if(!!err) {
            logger.debug('websocket send binary data failed: %j', err.stack);
            return;
        }
    });
    if (this.appMonitor) {
        this.appMonitor.pkgRecord(1, 1, msg.length); //外网网络包
    }
};
...
Socket.prototype.handshakeResponse = function(resp) {
    ...
    if (this.appMonitor) {
        this.appMonitor.pkgRecord(1, 1, resp.length); //外网网络包
    }
};

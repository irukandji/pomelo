...
var readHead = function(socket, data, offset) {
  ...
  if(socket.headOffset === socket.headSize) {
    ...
    if(size < 0 || size >= MAX_BODY_SIZE) {
      logger.warn('invalid body size: %s, the remote ip is %s', size, socket._socket.remoteAddress);
      dend = data.length;
      socket.close();
      return dend;
    }
    ...
    if(checkTypeData(socket.headBuffer[0])) {
      ...
      dend = data.length;
      logger.debug('close the connection with invalid head message, the remote ip is %s && port is %s', socket._socket.remoteAddress, socket._socket.remotePort);
      socket.close();
    }

...
var handleError = function(isGlobal, server, err, msg, session, resp, opts, cb) {
  ...
  if(!handler) {
    // logger.debug('no default error handler to resolve unknown exception. msg:%j, error:%j', msg, err);
  ...
  try {
    app.sysrpc[routeRecord.serverType].msgRemote.forwardMessage(
      ...
      function(err, resp, opts) {
        if(err) {
          logger.error('fail to process remote message: msg:%j, err:%j', msg, err);
        }
        finished = true;
        utils.invokeCallback(cb, err, resp, opts);
      }
    ...
    if(!finished) {
      logger.error('fail to forward remote message: msg:%j, err:%j', msg, err);

...
Server.prototype.start = function(cb) {
  ...
  this.masterConsole.on('disconnect', function(id, type, info, reason) {
    if (type === 'client') {
        adminLogger.info(util.format('[%s],[%s],[%s],[%s]', type, id, Date.now(), reason || 'disconnect'));
    } else {
  ...
  this.masterConsole.on('register', function(record) {
    starter.bindCpu(record.id, record.pid, record.sshHost);
  });
  ...
  this.masterConsole.on('admin-log', function(log, error) {
    if(error) {
      ...
      adminLogger.debug(JSON.stringify(log));
    }
  });
};

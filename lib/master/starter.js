...
starter.run = function(app, server, cb) {
  ...
  if (utils.isLocal(server.host)) {
    ...
    starter.sshrun(cmd, server.sshHost, cb);
  }
};
...
starter.kill = function(pids, servers) {
  ...
  for(var i = 0; i < servers.length; i++) {
    ...
    if(utils.isLocal(server.host)) {
      ...
      starter.sshrun(cmd, server.sshHost);
    }
  }
};
...
var spawnProcess = function(command, host, options, cb) {
  ...
  if(env === Constants.RESERVED.ENV_DEV) {
    child = cp.spawn(command, options, {windowsHide: true});
    ...
    child = cp.spawn(command, options, {detached: true, stdio: 'inherit', windowsHide: true});
    child.unref();
  }
